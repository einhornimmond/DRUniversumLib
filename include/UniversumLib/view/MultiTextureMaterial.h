...
namespace UniLib {
	namespace view {
		...
		public:
			...
			inline void setTexture(TexturePtr texture, size_t index) { assert(index < mTextureCount); mTextures[index] = texture; }
			void usingTexture(const char* filename, size_t index);
			inline TexturePtr getTexture(size_t index) { assert(index < mTextureCount); return mTextures[index]; }
		protected:
