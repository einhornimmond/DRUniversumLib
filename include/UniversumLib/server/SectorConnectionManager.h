...
namespace UniLib {
	...
	namespace server {
		...
		class UNIVERSUM_LIB_API SectorConnectionManager: public lib::Singleton, public lib::Thread
		{
		public:
			static SectorConnectionManager* const getInstance();

			DRReturn init();

            // login
            void login(const char* username, const char* password, DRNetServerConfig* accountServerConfig);
            inline bool isLogin() {return mLoginSuccessfully;}
			inline lib::CommandEventManager* getEventManager() {return mEventManager;}

            // default request
            //DRReturn sendRequest(DRNetRequest* request, RequestType type, model::SectorID* sektorID, CallbackCommand* callback = NULL);
		...
		private:
			...
			virtual ~SectorConnectionManager();

			static SectorConnectionManager* mpInstanz;
