...
namespace UniLib {
    ...
    namespace controller {
		...
		class UNIVERSUMLIB_EXPORT OctreeNode : public model::Sector
		{
		public:
			OctreeNode(model::SectorID* id, model::Node* parent = NULL);
			virtual ~OctreeNode();

			// update 
			virtual DRReturn move(float timeSinceLastFrame);

			inline model::Node* operator[] (DRVector3i index) {
				assert(index.x == 0 || index.x == 1);
				assert(index.y == 0 || index.y == 1);
				assert(index.z == 0 || index.z == 1);
				return mChilds[index.z*4+index.y*2+index.x];
			}
			inline model::Node* operator[] (u8 index) {
				assert(index >= 0 && index < 8);
				return mChilds[index];
			}

			// get name of sektor type
			virtual const char* getSectorType() { return "OctreeNode"; };			
